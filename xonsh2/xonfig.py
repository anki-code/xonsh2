...
For the configuration to take effect, you will need to restart xonsh2.
...
makes it easier to transition to and from xonsh2.
""".format(
    hr=HR
)
...
The following describes all xontribs that have been registered with xonsh2.
...
def make_xontrib(xontrib, package):
    """Makes a message and StoreNonEmpty node for a xontrib2."""
    name = xontrib2.get("name", "<unknown-xontrib-name>")
    msg = "\n{BOLD_CYAN}" + name + "{RESET}\n"
    if "url" in xontrib:
...
def _info(ns):
    ...
    data.extend([("xontrib2", xontribs_loaded())])
...
WELCOME_MSG = [
    ...
    "{{RED}}~/.xonshrc_2{{INTENSE_BLACK}} file to suppress the welcome screen)",
    "",
]
