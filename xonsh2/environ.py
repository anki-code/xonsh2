...
events.doc(
    "on_pre_spec_run_ls",
    """
on_pre_spec_run_ls(spec: xonsh2.built_ins.SubprocSpec) -> None
...
def xonsh_data_dir(env):
    ...
    xdd = os.path.expanduser(os.path.join(env.get("XDG_DATA_HOME"), "xonsh2"))
...
def xonsh_config_dir(env):
    ...
    xcd = os.path.expanduser(os.path.join(env.get("XDG_CONFIG_HOME"), "xonsh2"))
...
def default_xonshrc(env):
    ...
    xcdrc = os.path.join(xonsh_config_dir(env), "rc_2.xsh")
    if ON_WINDOWS:
        dxrc = (
            os.path.join(os_environ["ALLUSERSPROFILE"], "xonsh2", "xonshrc"),
            xcdrc,
            os.path.expanduser("~/.xonshrc_2"),
        )
    else:
        dxrc = ("/etc/xonshrc", xcdrc, os.path.expanduser("~/.xonshrc_2"))
...
def DEFAULT_VARS():
    dv = {
        ...
        "FOREIGN_ALIASES_SUPPRESS_SKIP_MESSAGE": Var(
            ...
            "because it already exists in xonsh2.",
        ...
        "FOREIGN_ALIASES_OVERRIDE": Var(
            ...
            "It cannot be set in the ``.xonshrc_2`` as loading of foreign aliases happens before"
            "``.xonshrc_2`` is parsed",
            doc_configurable=True,
        ),
        ...
        "TERM": Var(
            ...
            "options:\n\n"
            "* Set this from the program that launches xonsh2. On POSIX systems, \n"
        ...
        "XONSHRC": Var(
            ...
            doc_default=(
                "On Linux & Mac OSX: ``['/etc/xonshrc', '~/.config/xonsh/rc_2.xsh', '~/.xonshrc_2']``\n"
                "\nOn Windows: "
                "``['%ALLUSERSPROFILE%\\\\xonsh\\\\xonshrc', '~/.config/xonsh/rc_2.xsh', '~/.xonshrc_2']``"
            ),
        ),
        ...
        "XONSH_CACHE_SCRIPTS": Var(
            ...
            bool_to_str,
            True,
            "Controls whether the code for scripts run from xonsh2 will be cached"
...
def windows_foreign_env_fixes(ctx):
    ...
    # which messes up these environment variables for xonsh2.
