...
def update_context(name, ctx=None):
    """Updates a context in place from a xontrib2. If ctx is not provided,
...
def xontrib_data(ns):
    ...
    for name in installed_xontribs:
        if name not in data:
            loaded = f"xontrib2.{name}" in sys.modules
...
def _create_xontrib_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="xontrib2", description="Manages xonsh extensions"
    )
